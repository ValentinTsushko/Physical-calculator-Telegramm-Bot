...
def find_values(message, bot, translations, current_language):
    try:
        ...
        bot.register_next_step_handler(message, solution, bot, variables['a'], variables['b'],
        variables['c'], translations, current_language);
...
def solution(message, bot, a, b, c, translations, current_language):
    if message.text.lower() in ['ja', 'jö', 'jo', 'так', 'да', 'yes', '1', '+']:
        ...
        if (D > 0):
            ...
            bot.send_message(message.from_user.id, f"D = {D}, x_1 = {x_1} & x_2 = {x_2}");
        elif (D == 0):
            x = -b / (2*a);
